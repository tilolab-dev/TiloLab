  ...
  <div class="add_category_content">
    ...
    <div class="add_category">
      ...
      <div class="main_modal">
        <div class="add_icon">
          <span class="default_text"> Картинка сертифіката (не обов'язково) </span>
        ...
        <div class="category-name-wrap">
          <div class="add-description">
            <span class="default_text"> Назва сертифіката (українською) </span>

            <input
              v-model="certificateNameUk"
              class=""
              type="text"
        ...
        <div class="between_wrapper">
          <div class="add-description">
            <span class="default_text"> Номінал сертифіката </span>

            <input
              v-model="certificateAmount"
              class=""
              type="number"
              placeholder="Введіть номінал сертифіката"
            />
          </div>
          <div class="add-description">
            <span class="default_text"> Залишок </span>

            <input v-model="stockValue" class="" type="number" placeholder="Введіть залишок" />
          </div>
        </div>
      </div>
      <div class="category_description">
        <span class="default_text"> Опис сертифіката (українською) </span>
        <textarea
          id=""
          v-model="certificateTextUk"
          class=""
          placeholder="Введіть опис сертифіката"
          name="textUk"
          rows="3"
        />
      </div>
      ...
      <div class="checkbox_wrapper">
        <span class="default_text"> Показувати сертифікат на сайті? (опціонально) </span>

        <input id="categoryCheckbox" v-model="certificateVisible" type="checkbox" />
        <label for="categoryCheckbox" class="checkbox-elem"> </label>
      </div>

      <div class="button-group">
        <button class="closeModal" @click="resetForm">Скасувати</button>
        <button class="addItem" @click="addNewCertificate">Додати</button>
      </div>
    </div>
  </div>
</template>
...
const certificateNameUk = ref("");
const certificateNameEn = ref("");
const certificateNameRu = ref("");

const certificateTextUk = ref("");
const certificateAmount = ref(0);
...
const resetForm = () => {
  ...
  uploadStatus.value = "";
  certificateNameUk.value = "";
  certificateNameEn.value = "";
  certificateNameRu.value = "";
  certificateVisible.value = false;
  certificateTextUk.value = "";
};

const addNewCertificate = () => {
  loaderState.value = true;
  //   if (
  //     certificateNameUk.value.length < 1 &&
  //     certificateNameEn.value.length < 1 &&
  //     certificateNameRu.value.length < 1
  //   ) {
  //     emit("tooltip", {
  //       status: "error",
  ...
  //     });
  //     return;
  //   } else if (certificateNameUk.value.length < 1) {
  //     emit("tooltip", {
  //       status: "error",
  //       message: "Заповніть назву категорії Українською",
  //     });
  //     return;
  //   } else if (certificateNameEn.value.length < 1) {
  //     emit("tooltip", {
  //       status: "error",
  //       message: "Заповніть назву категорії Англійською",
  //     });
  //     return;
  //   } else if (certificateNameRu.value.length < 1) {
  //     emit("tooltip", {
  //       status: "error",
  ...
  //     });
  //     return;
  //   } else if (certificateTextUk.value.length < 1) {
  //     emit("tooltip", {
  //       status: "error",
  ...
  const translitString = transliterate(certificateNameUk.value);
  ...
  const uploadCategoryFile = async () => {
    ...
    try {
      formData.append(`certificate-images/${certificateNameUk.value}`, file.value);
      ...
      if (response.success) {
        ...
        alert("Помилка при завантаженні іконки сертифіката");
        //  alert("tooltip",
        // emit("tooltip", {
        //   status: "error",
  ...
  const uploadData = async (certificateIconPath) => {
    console.log("UPLOAD DATA", certificateIconPath);
    try {
      const res = await $fetch("/api/products/add-certificate", {
        method: "POST",
        body: {
          group: categoryName,
          visible: certificateVisible.value,
          name: certificateNameUk.value,
          description: certificateTextUk.value,
          amount: certificateAmount.value,
          stock: stockValue.value,
          certificateImg: certificateIconPath
        }
      });
      ...
      //       {
      //         language: "uk",
      //         title: certificateNameUk.value,
      //         description: certificateTextUk.value
      //       }
      //     ],
      //     categoryImg: certificateIconPath
      //   });
      //   return responce;
    } catch (error) {
  ...
  const uploadAllData = async () => {
    try {
      const certificateIconPath = await uploadCategoryFile();

      const uploadNewCategory = await uploadData(certificateIconPath);
      ...
      if (uploadNewCategory.statusCode === 200) {
        emit("tooltip", {
          status: "success",
          message: "Сертифікат успішно доданий"
        });
      }
      ...
      emit("tooltip", {
        status: "error",
        message: `Помилка при додаванні сертифіката ${error}`
      });
      loaderState.value = false;
      console.log(error);
    }
